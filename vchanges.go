...
// The locks are in the state.go file, working similarly as synchronized functions.
...
func (v *viewChangeManager) validateVoteMeMsgAndSendBackToCandidate(m VoteMe) {

	log.Debugf("received VoteMe from Server %d | msg: %+v", m.ServerId, m)
	...
	if receivedView < myView {
		log.Infof(" reject VoteMe message | Server %d's view: %d | myView: %d",
...
func (v *viewChangeManager) validateNewViewMsg(m NewView) {
	log.Debugf("received NewView from Server %d of view %d", m.ServerId, m.View)
